...
public class CholeskyDecomposition implements java.io.Serializable {
	private static final long serialVersionUID = 1233171098264484917L;
	 ...
	 * double[n][n]; isspd = (Arg.getColumnDimension() == n); // Main loop. for (int
	 * j = 0; j < n; j++) { double d = 0.0; for (int k = 0; k < j; k++) { double s =
	 * A[k][j]; for (int i = 0; i < k; i++) { s = s - R[i][k]*R[i][j]; } R[k][j] = s
	 * = s/R[k][k]; d = d + s*s; isspd = isspd & (A[k][j] == A[j][k]); } d = A[j][j]
	 * - d; isspd = isspd & (d > 0.0); R[j][j] = Math.sqrt(Math.max(d,0.0)); for
	 * (int k = j+1; k < n; k++) { R[k][j] = 0.0; } } }
	 * 
	 ...
	 * @exception RuntimeException         Matrix is not symmetric positive
	 *                                     definite.
	 */
