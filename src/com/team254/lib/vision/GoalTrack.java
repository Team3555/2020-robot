...
public class GoalTrack {
    ...
    public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
        ...
        if (distance < AluminatiData.maxTrackerDistance) {
    ...
    synchronized void pruneByTime() {
        double delete_before = Timer.getFPGATimestamp() - AluminatiData.maxGoalTrackAge;
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            for (Map.Entry<Double, Pose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > AluminatiData.maxGoalTrackSmoothingTime) {
                    continue;
                }
                ++num_samples;
    ...
    public synchronized double getStability() {
        return Math.min(1.0, mObservedPositions.size() / (AluminatiData.cameraFrameRate * AluminatiData.maxGoalTrackAge));
    }
